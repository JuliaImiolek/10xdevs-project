...
import { TEXT_INPUT_MIN_LENGTH, MANUAL_FRONT_MAX, MANUAL_BACK_MAX } from "./pages/GeneratePage";

test.describe("Strona Generuj fiszki", () => {
  test.describe("ochrona ścieżki", () => {
    test("niezalogowany użytkownik jest przekierowany na logowanie z redirectTo", async ({ page }) => {
  ...
  test.describe("jako zalogowany użytkownik", () => {
    ...
    test("wyświetla nagłówek, pole tekstowe, przycisk generowania i formularz ręcznej fiszki", async ({ page }) => {
    ...
    test("walidacja tekstu źródłowego: tekst krótszy niż minimum po blurze pokazuje błąd", async ({ page }) => {
      const generatePage = new GeneratePage(page);
      await generatePage.goto();
      ...
      await expect(generatePage.errorNotification).toContainText(String(TEXT_INPUT_MIN_LENGTH));
    });

    test("walidacja tekstu źródłowego: klik Generuj przy pustym polu pokazuje błąd", async ({ page }) => {
      const generatePage = new GeneratePage(page);
      await generatePage.goto();
    ...
    test("formularz ręcznej fiszki: submit z pustymi polami pokazuje błędy wymaganych pól", async ({ page }) => {
      const generatePage = new GeneratePage(page);
      await generatePage.goto();
    ...
    test("formularz ręcznej fiszki: poprawne wypełnienie i submit pokazuje komunikat sukcesu", async ({ page }) => {
      const generatePage = new GeneratePage(page);
      await generatePage.goto();
    ...
    test("formularz ręcznej fiszki: pole Przód powyżej limitu znaków pokazuje błąd walidacji", async ({ page }) => {
      const generatePage = new GeneratePage(page);
      await generatePage.goto();
      ...
      await expect(page.getByRole("alert")).toContainText(`Maksymalnie ${MANUAL_FRONT_MAX} znaków`);
    });

    test("formularz ręcznej fiszki: pole Tył powyżej limitu znaków pokazuje błąd walidacji", async ({ page }) => {
      const generatePage = new GeneratePage(page);
      await generatePage.goto();
      ...
      await expect(page.getByRole("alert")).toContainText(`Maksymalnie ${MANUAL_BACK_MAX} znaków`);
    });

    test("błąd API generacji: odpowiedź 500 pokazuje komunikat błędu w UI", async ({ page }) => {
    ...
    test("przycisk Generuj fiszki jest zablokowany podczas ładowania", async ({ page }) => {
